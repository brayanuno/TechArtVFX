...
public class DragonPath : MonoBehaviour
{
    ...
    protected void AlterInfluencerVisibility(bool show, List<PathNode> path)
    {
        foreach (PathNode node in path)
        {
            node.influencer1.GetComponent<MeshRenderer>().enabled =
    ...
    private float PositionDragonPart(Transform transPos, Transform transMesh, Transform transParent, Quaternion rotationCorrection, float pos, float alteredWaveTimer, float segmentPos)
    {
        ...
        float posBetweenBezierNodes = (posBetweenNodes * lineResolution) % 1f;

        int firstPathNode = (int)Mathf.Floor(pos * dragonIdlePath.Count) % dragonIdlePath.Count;
        int firstBezierNode = (int)Mathf.Floor(posBetweenNodes * lineResolution) % lineResolution;

        int secondPathNode = firstPathNode;
        int secondBezierNode = (firstBezierNode + 1);

        // keep in bounds
        if (secondBezierNode >= lineResolution)
        {
            secondBezierNode %= lineResolution;
    ...
    private void Start()
    {
        ...
        GenerateBezierLine(lineResolution, dragonIdlePath);
    ...
    private void Update()
    {
        ...
        {
            float speed = speedCurve.Evaluate(pathPos) * this.speed;

            waveTimer = (waveTimer + Time.deltaTime * speed) % 1f;

            float distance = PositionDragonPart(dragonSkeletonStart.transform.parent, dragonSkeletonStart.transform, null, dragonHeadRotationCorrection, pathPos, waveTimer, 0);

            pathPos = (pathPos + Time.deltaTime * (speed / distance)) % 1f;
            ...
            RecursiveSegmentUpdating(
                dragonSkeletonStart.transform.GetChild(0), pathPos, distance, alteredWaveTimer, 1f / segmentCount);
        }
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        for (int i = 0; i < dragonIdlePath.Count; ++i)
        {
            ...
            if (node.lineToNextNode != null)
            {
                ...
                for (int j = 0; j < lineResolution - 1; ++j)
                {
                    Gizmos.DrawLine(
                ...
                Gizmos.DrawLine(
                    node.lineToNextNode[lineResolution - 1],
